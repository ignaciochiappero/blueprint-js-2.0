...
let floor_textures = floor_textures_json;
let floor_texture_keys = Object.keys(floor_textures);

let wall_textures = wall_textures_json;
let wall_texture_keys = Object.keys(wall_textures);

let blueprint3d = null;

let app_parent = document.getElementById('bp3d-js-app');
let sidebarWrapper = document.getElementById('bp3d-sidebar');

function createSidebarPanel(title) {
    let panel = QuickSettings.create(0, 0, title, sidebarWrapper);
    panel._panel.classList.add('bs-panel');
...
if (!opts.widget) {
    uxInterface = createSidebarPanel('BlueprintJS');

    settingsViewer2d = createSidebarPanel('Viewer 2D');
    settingsSelectedCorner = createSidebarPanel('Corner');
    settingsSelectedWall = createSidebarPanel('Wall');
    settingsSelectedRoom = createSidebarPanel('Room');

    settingsViewer3d = createSidebarPanel('Viewer 3D');
    settingsSelectedWall3D = createSidebarPanel('Wall (3D)');
    settingsSelectedRoom3D = createSidebarPanel('Room (3D)');
